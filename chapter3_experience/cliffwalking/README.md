...
```
**python 3.8.5, Tensorflow 2.4.1, numpy 1.19.5, matplotlib 3.6.3, gym 0.25.2, pygame 2.6.1** 권장

**Python 3.8 과 3.11 에서 동작을 확인했습니다.** 더 최신 버전에서는 `gym` 설치나 실행이
실패할 수 있습니다(2022년 이후 유지보수 중단). 
그런 경우 파이썬을 3.11 로 맞추거나, 아래처럼 버전을 직접 지정해 설치하세요.
...
```
python play.py            # 1. 먼저 손으로 움직여 볼 것
...
조작 방법은 같지만 **키는 창이 아니라 터미널에서 입력**합니다. 창을 클릭하면 키가 동작하지 않습니다.
...
**1. `select_action(Q, s, eps, rng)`** — ε-greedy  `eps` 확률로 무작위 선택 방식으로 구현해 보세요.

**2. `update_q(Q, s, a, r, s2, terminated)`** 을 구현하세요.
...
코드 완성 후 python 코드가 있는 폴더에서 다음 방식으로 실행 할 수 있습니다.
```
python q_learning.py            # 학습 코드 실행 및 학습된 정책 확인
...
`--watch` 는 **다 학습된 뒤의 정책**만 재생합니다. 

학습이 진행되는 과정을 보려면 `watch_training.py` 를 실행하세요. 옵션 없이 그냥 실행하면 됩니다.
...
```
이 값들은 `watch_training.py`  파일 상단에서 바꿀 수 있습니다. 바꿔가면서 실행해보세요.
